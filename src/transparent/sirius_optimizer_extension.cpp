                           ...
                           duckdb::unique_ptr<duckdb::LogicalOperator>& plan)
{
  ...
  // filter_pushdown and LogicalGet's dynamic_filters are not in the serialization schema, so
  // they're null on the copy by default. preserve_dynamic_filter_metadata walks both trees in
  // parallel and re-attaches them, sharing the DynamicTableFilterSet shared_ptrs to preserve
  // the route-key pointer identity that downstream Sirius wiring relies on.
  try {
    auto plan_copy = plan->Copy(context);
    detail::preserved_counts counts;
    detail::preserve_dynamic_filter_metadata(*plan, *plan_copy, counts);
