...
/**
 * @brief Read one coherent filter-cascade policy from a DuckDB connection.
 *
 * The extension option is session-scoped. Call this at a planning/bind boundary and carry
 * the returned value with the physical operator; executor threads must not read the connection.
 * If Sirius options are not registered, the policy's built-in defaults are returned.
 *
 * @param context Connection whose current session setting is copied
 * @return Immutable filter-cascade policy for a physical plan
 */
[[nodiscard]] filter_cascade_policy filter_cascade_policy_from_context(
