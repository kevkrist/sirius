...
constexpr int k_status_projection_overflow   = 32;
...
struct q1_projection_spec {
  q1_register_sum_spec inputs[4];
  int64_t one_discount;
  int64_t one_tax;
  int8_t physical_sources[k_q1_register_sums];
};
...
__device__ __forceinline__ bool checked_add_int64_value(int64_t lhs, int64_t rhs, int64_t& result)
{
...
__device__ __forceinline__ bool checked_sub_int64_value(int64_t lhs, int64_t rhs, int64_t& result)
{
  if (rhs > 0 && lhs < std::numeric_limits<int64_t>::min() + rhs) { return false; }
  if (rhs < 0 && lhs > std::numeric_limits<int64_t>::max() + rhs) { return false; }
  result = lhs - rhs;
...
__device__ __forceinline__ bool checked_mul_int64_value(int64_t lhs, int64_t rhs, int64_t& result)
{
  auto const low = static_cast<uint64_t>(lhs) * static_cast<uint64_t>(rhs);
  auto const high =
    static_cast<int64_t>(__mul64hi(static_cast<long long>(lhs), static_cast<long long>(rhs)));
  result = static_cast<int64_t>(low);
  return high == (result < 0 ? int64_t{-1} : int64_t{0});
}

__device__ __forceinline__ bool fits_decimal_precision(int64_t value, int64_t magnitude_limit)
{
  return value > -magnitude_limit && value < magnitude_limit;
}
...
__global__ void accumulate_q1_projection_register_private_kernel(
  key_spec const* key_specs,
  int64_t rows,
  int16_t const* dense_map,
  q1_projection_spec const* projection_spec,
  int64_t* thread_partials,
  int32_t* status,
  bool sampled_preflight)
{
  ...
  bool register_overflow   = false;
  bool projection_overflow = false;
  bool bad_state           = false;
  bool sampled_key_miss    = false;
  auto const thread        = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  auto const total_threads = static_cast<int64_t>(gridDim.x) * blockDim.x;
  auto const& projection   = *projection_spec;
  for (int64_t row = thread; row < rows; row += total_threads) {
    auto const packed = packed_key(key_specs, 2, row, status);
    if (packed < 0) { continue; }
    auto const group = static_cast<int>(dense_map[packed]);
    if (group < 0) {
      sampled_key_miss |= sampled_preflight;
    ...
    int64_t const independent =
      projection.inputs[0]
        .input_data[projection.inputs[0].input_offset + static_cast<cudf::size_type>(row)];
    int64_t const price =
      projection.inputs[1]
        .input_data[projection.inputs[1].input_offset + static_cast<cudf::size_type>(row)];
    int64_t const discount =
      projection.inputs[2]
        .input_data[projection.inputs[2].input_offset + static_cast<cudf::size_type>(row)];
    int64_t const tax =
      projection.inputs[3]
        .input_data[projection.inputs[3].input_offset + static_cast<cudf::size_type>(row)];

    int64_t discount_factor{};
    int64_t discounted_price{};
    int64_t tax_factor{};
    int64_t charge{};
    bool arithmetic_ok = fits_decimal_precision(independent, int64_t{1000000000000000}) &&
                         fits_decimal_precision(price, int64_t{1000000000000000}) &&
                         fits_decimal_precision(discount, int64_t{1000000000000000}) &&
                         fits_decimal_precision(tax, int64_t{1000000000000000});
    arithmetic_ok = arithmetic_ok &&
                    checked_sub_int64_value(projection.one_discount, discount, discount_factor) &&
                    fits_decimal_precision(discount_factor, int64_t{10000000000000000}) &&
                    checked_mul_int64_value(price, discount_factor, discounted_price) &&
                    fits_decimal_precision(discounted_price, int64_t{1000000000000000000}) &&
                    checked_add_int64_value(projection.one_tax, tax, tax_factor) &&
                    fits_decimal_precision(tax_factor, int64_t{10000000000000000}) &&
                    checked_mul_int64_value(discounted_price, tax_factor, charge) &&
                    fits_decimal_precision(charge, int64_t{1000000000000000000});
    if (!arithmetic_ok) {
      projection_overflow = true;
      continue;
    }

    int64_t physical_values[k_q1_register_sums];
#pragma unroll
    for (int physical = 0; physical < k_q1_register_sums; ++physical) {
      switch (static_cast<tiny_domain_q1_value_source>(projection.physical_sources[physical])) {
        case tiny_domain_q1_value_source::independent_sum:
          physical_values[physical] = independent;
          break;
        case tiny_domain_q1_value_source::price: physical_values[physical] = price; break;
        case tiny_domain_q1_value_source::discount: physical_values[physical] = discount; break;
        case tiny_domain_q1_value_source::discounted_price:
          physical_values[physical] = discounted_price;
          break;
        case tiny_domain_q1_value_source::charge: physical_values[physical] = charge; break;
        default:
          physical_values[physical] = 0;
          bad_state                 = true;
          break;
      }
    }
    auto const value0 = physical_values[0];
    auto const value1 = physical_values[1];
    auto const value2 = physical_values[2];
    auto const value3 = physical_values[3];
    auto const value4 = physical_values[4];
  ...
  if (register_overflow) { atomicOr(status, k_status_register_overflow); }
  if (projection_overflow) { atomicOr(status, k_status_projection_overflow); }
  if (bad_state) { atomicOr(status, k_status_bad_state); }
  if (sampled_key_miss) { atomicOr(status, k_status_sample_miss); }
...
[[nodiscard]] std::string status_reason(int status)
{
  ...
  if ((status & k_status_projection_overflow) != 0) {
    return "fused projection arithmetic exceeded its exact DECIMAL64 carrier";
  }
...
tiny_domain_grouped_aggregate_attempt try_tiny_domain_q1_projection_aggregate(
  cudf::table_view input,
  ...
  rmm::device_async_resource_ref mr)
{
  ...
  if (input.num_rows() == 0) { return fallback("empty input uses the sequential projection path"); }
  if (input.num_rows() > std::numeric_limits<int32_t>::max()) {
    return fallback("input row count exceeds the representative-row carrier");
  }
  if (input.num_columns() != 6) {
    return fallback("fused Q1 raw input schema does not contain exactly six columns");
  }
  if (aggregates.size() != k_q1_logical_states) {
    return fallback("fused Q1 aggregate requires exactly eleven local states");
  }

  constexpr std::array<tiny_domain_q1_value_source, 5> expected_physical_sources{
  ...
  constexpr std::array<int8_t, k_q1_logical_states> expected_logical_map{
    0, 1, 2, 3, 0, 5, 1, 5, 4, 5, 5};
  if (plan.physical_value_sources != expected_physical_sources) {
    return fallback("fused Q1 physical sources do not match the exact Q1 layout");
  }
  if (plan.logical_to_physical != expected_logical_map) {
    return fallback("fused Q1 logical states do not match the exact Q1 layout");
  }

  auto const raw_decimal = sirius::logical_type::make_decimal(15, 2);
  std::array<bool, 5> seen_sources{};
  std::array<int, 5> sum_multiplicity{};
  int count_valid_states = 0;
  int count_all_states   = 0;
  for (std::size_t physical = 0; physical < plan.physical_value_sources.size(); ++physical) {
    auto const source       = plan.physical_value_sources[physical];
    auto const source_index = static_cast<int>(source);
    if (source_index < 0 || source_index >= static_cast<int>(seen_sources.size()) ||
        seen_sources[static_cast<std::size_t>(source_index)]) {
      return fallback("fused Q1 plan has an invalid or duplicated physical source");
    }
    seen_sources[static_cast<std::size_t>(source_index)] = true;
    uint8_t const precision = source == tiny_domain_q1_value_source::discounted_price ||
                                  source == tiny_domain_q1_value_source::charge
                                ? uint8_t{18}
                                : uint8_t{15};
    uint8_t const scale     = source == tiny_domain_q1_value_source::discounted_price ? uint8_t{4}
                              : source == tiny_domain_q1_value_source::charge         ? uint8_t{6}
                                                                                      : uint8_t{2};
    if (plan.physical_sum_types[physical] != sirius::logical_type::make_decimal(precision, scale)) {
      return fallback("fused Q1 SUM input type does not match its exact DECIMAL role");
    }
  }
  for (std::size_t logical = 0; logical < aggregates.size(); ++logical) {
    auto const physical = static_cast<int>(plan.logical_to_physical[logical]);
    switch (aggregates[logical]) {
      case cudf::aggregation::Kind::SUM:
        if (physical < 0 || physical >= k_q1_register_sums) {
          return fallback("fused Q1 SUM has an invalid physical-state mapping");
        }
        ++sum_multiplicity[static_cast<std::size_t>(physical)];
        break;
      case cudf::aggregation::Kind::COUNT_VALID:
        if (physical != k_q1_register_sums) {
          return fallback("fused Q1 COUNT_VALID does not map to the shared count state");
        }
        ++count_valid_states;
        break;
      case cudf::aggregation::Kind::COUNT_ALL:
        if (physical != k_q1_register_sums) {
          return fallback("fused Q1 COUNT_ALL does not map to the shared count state");
        }
        ++count_all_states;
        break;
      default: return fallback("fused Q1 aggregate kind is not SUM or COUNT");
    }
  }
  auto sorted_sum_multiplicity = sum_multiplicity;
  std::sort(sorted_sum_multiplicity.begin(), sorted_sum_multiplicity.end());
  if (sorted_sum_multiplicity != std::array<int, 5>{1, 1, 1, 2, 2} || count_valid_states != 3 ||
      count_all_states != 1) {
    return fallback("fused Q1 local-state multiplicities do not match the exact Q1 shape");
  }

  std::array<bool, 6> distinct_indices{};
  std::vector<cudf::column_view> group_columns;
  std::vector<key_spec> host_key_specs;
  group_columns.reserve(2);
  host_key_specs.reserve(2);
  for (auto const index : plan.group_input_idx) {
    if (index < 0 || index >= input.num_columns() ||
        index >= static_cast<int>(distinct_indices.size()) ||
        distinct_indices[static_cast<std::size_t>(index)]) {
      return fallback("fused Q1 grouping-key index is invalid or aliases another raw input");
    }
    distinct_indices[static_cast<std::size_t>(index)] = true;
    auto const column                                 = input.column(index);
    if (column.type().id() != cudf::type_id::STRING || column.null_count() != 0) {
      return fallback("fused Q1 grouping key is not STRING");
    }
    group_columns.push_back(column);
    host_key_specs.push_back(make_key_spec(column, stream));
  }

  q1_projection_spec host_projection_spec{};
  host_projection_spec.one_discount = 100;
  host_projection_spec.one_tax      = 100;
  for (std::size_t raw = 0; raw < plan.decimal_input_idx.size(); ++raw) {
    auto const index = plan.decimal_input_idx[raw];
    if (plan.decimal_input_types[raw] != raw_decimal || index < 0 || index >= input.num_columns() ||
        index >= static_cast<int>(distinct_indices.size()) ||
        distinct_indices[static_cast<std::size_t>(index)]) {
      return fallback("fused Q1 raw DECIMAL input index or logical type is invalid");
    }
    distinct_indices[static_cast<std::size_t>(index)] = true;
    auto const column                                 = input.column(index);
    if (column.type() != cudf::data_type{cudf::type_id::DECIMAL64, -2} ||
        column.null_count() != 0) {
      return fallback("fused Q1 raw input is not non-null DECIMAL64(15,2)");
    }
    host_projection_spec.inputs[raw] =
      q1_register_sum_spec{column.head<int64_t>(), column.offset()};
  }
  for (std::size_t physical = 0; physical < plan.physical_value_sources.size(); ++physical) {
    host_projection_spec.physical_sources[physical] =
      static_cast<int8_t>(plan.physical_value_sources[physical]);
  }

  rmm::device_uvector<key_spec> device_key_specs(host_key_specs.size(), stream, mr);
  rmm::device_uvector<int32_t> representatives(k_max_key_domain, stream, mr);
  rmm::device_uvector<int32_t> device_status(1, stream, mr);
  CUDF_CUDA_TRY(cudaMemcpyAsync(device_key_specs.data(),
                                host_key_specs.data(),
                                host_key_specs.size() * sizeof(key_spec),
  ...
  std::vector<int32_t> host_representatives(k_max_key_domain);
  int32_t host_status{0};
  auto run_preflight = [&](int64_t rows) {
    CUDF_CUDA_TRY(cudaMemsetAsync(
      representatives.data(), 0xff, representatives.size() * sizeof(int32_t), stream.value()));
    CUDF_CUDA_TRY(cudaMemsetAsync(device_status.data(), 0, sizeof(int32_t), stream.value()));
    auto const grid = resident_grid_for(rows, preflight_keys_kernel);
    preflight_keys_kernel<<<grid, k_block_size, 0, stream.value()>>>(
      device_key_specs.data(), 2, rows, representatives.data(), device_status.data());
    CUDF_CUDA_TRY(cudaGetLastError());
    CUDF_CUDA_TRY(cudaMemcpyAsync(host_representatives.data(),
                                  representatives.data(),
                                  host_representatives.size() * sizeof(int32_t),
  ...
  bool const prefix_is_sample = input.num_rows() > k_q1_preflight_sample_rows;
  run_preflight(prefix_is_sample ? k_q1_preflight_sample_rows : input.num_rows());
  if (host_status != 0) { return fallback(status_reason(host_status)); }
  auto sample_groups                = static_cast<int>(std::count_if(host_representatives.begin(),
                                                      host_representatives.end(),
                                                      [](int32_t row) { return row >= 0; }));
  bool const used_sampled_preflight = prefix_is_sample && sample_groups == k_q1_register_groups;
  if (prefix_is_sample && !used_sampled_preflight) {
  ...
  std::vector<cudf::size_type> selected_rows;
  std::vector<int16_t> host_dense_map(k_max_key_domain, int16_t{-1});
  selected_rows.reserve(k_q1_register_groups);
  for (int packed = 0; packed < k_max_key_domain; ++packed) {
    if (host_representatives[packed] < 0) { continue; }
    if (selected_rows.size() == k_q1_register_groups) {
      return fallback("fused Q1 runtime key cardinality exceeds four groups");
    }
    host_dense_map[packed] = static_cast<int16_t>(selected_rows.size());
    selected_rows.push_back(host_representatives[packed]);
  }
  if (selected_rows.size() != k_q1_register_groups) {
    return fallback("fused Q1 runtime key cardinality is not exactly four groups");
  }

  rmm::device_uvector<int16_t> device_dense_map(k_max_key_domain, stream, mr);
  rmm::device_uvector<q1_projection_spec> device_projection_spec(1, stream, mr);
  rmm::device_uvector<int8_t> device_logical_map(plan.logical_to_physical.size(), stream, mr);
  CUDF_CUDA_TRY(cudaMemcpyAsync(device_dense_map.data(),
                                host_dense_map.data(),
                                host_dense_map.size() * sizeof(int16_t),
                                cudaMemcpyHostToDevice,
                                stream.value()));
  CUDF_CUDA_TRY(cudaMemcpyAsync(device_projection_spec.data(),
                                &host_projection_spec,
                                sizeof(host_projection_spec),
                                cudaMemcpyHostToDevice,
                                stream.value()));
  CUDF_CUDA_TRY(cudaMemcpyAsync(device_logical_map.data(),
                                plan.logical_to_physical.data(),
                                plan.logical_to_physical.size() * sizeof(int8_t),
                                cudaMemcpyHostToDevice,
                                stream.value()));
  ...
  for (std::size_t logical = 0; logical < aggregates.size(); ++logical) {
    ...
    if (kind == cudf::aggregation::Kind::SUM) {
      auto const scale =
        plan.physical_sum_types[static_cast<std::size_t>(physical)].decimal_scale();
      output_type = cudf::data_type{cudf::type_id::DECIMAL128, -static_cast<int32_t>(scale)};
    } else {
      output_type = cudf::data_type{
        kind == cudf::aggregation::Kind::COUNT_ALL ? cudf::type_id::INT64 : cudf::type_id::INT32};
    }
    auto output = cudf::make_fixed_width_column(
      output_type, k_q1_register_groups, cudf::mask_state::UNALLOCATED, stream, mr);
    auto mutable_output = output->mutable_view();
    host_aggregate_specs.push_back(aggregate_spec{
      static_cast<int32_t>(kind),
      static_cast<int32_t>(kind == cudf::aggregation::Kind::SUM ? cudf::type_id::DECIMAL64
                                                                : cudf::type_id::INT8),
      static_cast<int32_t>(output_type.id()),
      0,
      nullptr,
      0,
      nullptr,
  ...
  CUDF_CUDA_TRY(cudaMemsetAsync(device_status.data(), 0, sizeof(int32_t), stream.value()));
  auto const grid =
    resident_grid_for(input.num_rows(), accumulate_q1_projection_register_private_kernel);
  auto const partial_threads = static_cast<std::size_t>(grid) * k_block_size;
  rmm::device_uvector<int64_t> thread_partials(
    partial_threads * k_q1_register_groups * k_q1_register_states, stream, mr);
  constexpr auto physical_entries = k_q1_register_groups * k_q1_register_states;
  rmm::device_uvector<wide_value> physical_results(physical_entries, stream, mr);

  accumulate_q1_projection_register_private_kernel<<<grid, k_block_size, 0, stream.value()>>>(
    device_key_specs.data(),
    input.num_rows(),
    device_dense_map.data(),
    device_projection_spec.data(),
    thread_partials.data(),
    device_status.data(),
    used_sampled_preflight);
  CUDF_CUDA_TRY(cudaGetLastError());
  reduce_q1_register_partials_kernel<<<physical_entries, k_block_size, 0, stream.value()>>>(
  ...
  stream.synchronize();
  if ((host_status & k_status_overflow) != 0) {
    throw std::overflow_error("fused Q1 aggregate invariant failed: " + status_reason(host_status));
  }
  if ((host_status & k_status_bad_state) != 0) {
    throw std::runtime_error("fused Q1 aggregate invariant failed: " + status_reason(host_status));
  }
  if (host_status != 0) { return fallback(status_reason(host_status)); }
