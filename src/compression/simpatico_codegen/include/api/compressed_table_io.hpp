...
// C++-native .hpln v10 writer/reader for compressed_table.
//
...
//
// Splits the .hpln v10 stream into its two natural regions so the caller can keep
...
/// Build the .hpln v10 header for @p table into @p out_header and enumerate every
...
/// Header-only serialization over a live staged candidate. The inspection and every returned
/// device pointer remain valid only while its stage is active.
