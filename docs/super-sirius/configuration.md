...
sirius:
  ...
  operator_params:
    ...
    dynamic_filter_publication_scheme: root_pipelined  # accumulated-Bloom reduce+replicate: root_pipelined | root_serial
    dynamic_filter_publication_chunk_bytes: 0  # root_pipelined chunk; 0 = clamp(filter_bytes / 8, 2 MiB, 8 MiB)
    enable_dynamic_zone_map_filter: false  # optional parquet-read/native-post-decode min/max
    dynamic_filter_domain_coverage_threshold: 0.45  # skip all filters for proven-unique keys meeting known-domain coverage
    dynamic_filter_domain_evidence: catalog_and_pinned  # gate evidence: catalog_and_pinned | catalog_only
    ...
    enable_dynamic_filter_in_scan: true  # membership masks inside GPU_SCAN; one survivor gather materializes the split
    dynamic_filter_mask_kernel: fused  # scan-side mask pass: fused (one kernel per split) | cascade (probe + AND pass per filter)
...
| `dynamic_filter_publication_scheme` | `root_pipelined` | How the multi-partition accumulated Bloom is reduced onto its root GPU (the final contributor) and replicated. `root_pipelined`: chunk-major pipeline — per chunk, every remote partial slice is pulled into a double-buffered root scratch on a copy stream, OR-ed on a second root stream, and pulled by every target as soon as that chunk's OR event fires, so ingress, OR and egress overlap on the root's full-duplex PCIe link with one host wait per target (about 2x faster than serial on 4 GPUs; needs `memory.gpu.enable_pool_peer_access` for direct DMA). `root_serial`: pull-and-OR every partial on one root stream, drain it, then broadcast and wait per target. Pipelining requires direct peer DMA on every (target, root) pair in both directions and otherwise falls back to serial with an INFO log. Both schemes produce identical filters. |
| `dynamic_filter_publication_chunk_bytes` | 0 | Chunk size for `root_pipelined`. `0` selects `clamp(filter_bytes / 8, 2 MiB, 8 MiB)` (4–8 MiB is optimal for filters >= 90 MB, 2 MiB near 20 MB; 64 MiB chunks lose the overlap). Rounded up to the 32-byte Bloom block and capped at the filter footprint. Root scratch is `2 * (GPUs - 1) * chunk` bytes for the duration of the publication. |
...
| `dynamic_filter_domain_coverage_threshold` | 0.45 | Positive finite threshold. Before constructing either a membership filter or zone map, skip the key when the complete build covers at least this fraction of the key's unfiltered base-table row bound. Applies only to build keys proven unique in their base relation (see `dynamic_filter_domain_evidence`). Values above 1.0 disable the gate; exactly 1.0 fires only at full coverage. The default is tuned on the multi-GPU cost of a global Bloom: a build covering about half of the domain keeps about half of the probe rows, and its `add`, reduction and PCIe replication then outweigh the halved downstream work (TPC-H q21 `o_orderkey`, coverage 0.487, -31 ms of 163 on 4x L40S), while builds near 0.30 still pay off. A marginal-benefit gate is the principled successor. |
| `dynamic_filter_domain_evidence` | `catalog_and_pinned` | Where the gate takes its domain and uniqueness evidence. `catalog_only`: `seq_scan` row bounds and PRIMARY KEY constraints (parquet scans never qualify). `catalog_and_pinned`: additionally, a parquet-family scan whose file set is pinned takes the pinned entry's exact row count as its domain, and a build key is unique when the matching pinned entry declared it through `pin_table(..., unique_cols => [...])`. Pinned evidence is consumed only by dynamic-filter key admission (a wrong declaration can only skip a filter the join then answers exactly), never by join-kind selection. Missing evidence disables the gate for that key. |
...
| `enable_dynamic_filter_in_scan` | true | Apply the published membership dynamic filters inside `GPU_SCAN` for parquet-backed scans: the residual row filter and every visible membership mask are evaluated on the split before it is materialized (on its stored carriers, so a pinned narrow carrier is widened per probe value rather than cast first), ANDed, and the split is materialized by one gather of the survivors. The `DYNAMIC_FILTER` operator shares the scan's selectivity gate and only applies filters published after the scan's snapshot. Off restores copy-then-filter: the scan materializes every row (a deep copy for a pinned chunk) and the operator filters afterwards. Requires `enable_dynamic_filter`. Trade-off: on, a split allocates one BOOL8 mask per applied filter (1 byte/row each) instead of a full-width copy; a scan whose filters keep almost everything pays the mask kernels for a gather that saves nothing, which the shared gate then disables as before. |
| `dynamic_filter_mask_kernel` | `fused` | How a scan that applies membership filters itself (`enable_dynamic_filter_in_scan`) forms a split's survivor mask. `fused`: one kernel per split walks every row through the residual row filter and the membership probes (Bloom, hash IN-list, small IN-list) in the gate's cascade order with early-out, writing the conjunction and the per-step survivor counts in a single pass — no intermediate mask columns, one launch instead of 2 + 2K for K filters; the per-filter marginal keep ratios the gate records are unchanged. A lone membership filter without a residual keeps its plain probe kernel and takes its count from the gather (the fused pass would only add a count reduction to the same probe); a residual alone is gathered directly. `cascade`: one stencilled probe kernel per filter plus a streaming AND + count pass after the residual and after each mask (the pre-fusion shape). Both produce identical survivors and gate decisions; `cascade` is the control for A/B measurement and the fallback for a filter kind without a fused device probe. Trade-off: the fused kernel is bound by the residual/key/mask streams and the membership structures' L2 sectors, so it saves the most where several filters or a residual apply to a large split (TPC-H q3/q8/q21 lineitem, 30-50 M rows: 2-5 DRAM passes → 1) and nothing on a split with one filter and no residual. |
...
`pin_table` also accepts `unique_cols => [...]`, a list of pinned columns the caller declares
unique. The declaration is stored on the pinned entry and read only by the dynamic-filter
domain-coverage gate under `dynamic_filter_domain_evidence: catalog_and_pinned` (see
[Dynamic Filters](#dynamic-filters) and [Pinned tables](scan.md#pinned-tables)).
