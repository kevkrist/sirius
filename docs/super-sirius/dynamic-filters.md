...
- The domain-coverage gate skips the key before either filter is built when a proven-unique build key covers at least `dynamic_filter_domain_coverage_threshold` of its known base-table domain. The build key is traced through value-preserving row subsets (projection references, filters, ordering, limits, distinct, single-grouping-set groups, semi/anti/mark joins) to its base scan; the domain is that scan's unfiltered row count and uniqueness is that scan column's. Under `dynamic_filter_domain_evidence: catalog_only` both come from the DuckDB catalog (`seq_scan` row bounds, PRIMARY KEY), so parquet builds never qualify; the default `catalog_and_pinned` additionally reads the exact row count of a pinned parquet file set and the columns `pin_table(..., unique_cols => [...])` declared unique. That declaration feeds this gate only: a wrong one can skip a filter the join then answers exactly, and it never selects `cudf::distinct_hash_join`. The gate runs in the accumulator constructor before any Bloom `add`, so a skipped multi-partition key costs neither construction nor replication. The default threshold (0.45) is tuned on the multi-GPU Bloom cost: a build covering half of its domain keeps about half of the probe rows, and the `add`, reduction and PCIe replication of its Bloom outweigh the halved downstream work (TPC-H q21 `o_orderkey`, coverage 0.487 at SF100), while builds at 0.30 still pay off.
...
| `dynamic_filter_domain_coverage_threshold` | `0.45` | Skip a proven-unique key at or above this known-domain coverage; values above `1.0` disable the gate |
| `dynamic_filter_domain_evidence` | `catalog_and_pinned` | Evidence the gate may use: `catalog_only` (seq_scan bounds + PRIMARY KEY) or additionally pinned parquet row counts and `pin_table` `unique_cols` declarations |
