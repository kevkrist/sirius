...
- `dynamic_filter_build_priority` (`legacy` or `off`, default **off**) — controls the scheduler's soft preference for queued tasks that feed dynamic-filter builds. `off` preserves normal queue order without disabling filter construction, publication, application, or telemetry. The value is snapshotted when each query is prepared, so a `SET` applies to subsequent queries. For the first release with this default, restore the previous behavior without rebuilding via `SET dynamic_filter_build_priority='legacy'` or the equivalent YAML setting.

Release note (#1124): dynamic-filter build-feeder priority now defaults to `off` after the SF=300 acceptance matrix showed no wall-time or resident-memory regression. The `legacy` rollback remains available for one release.
