...
2. If the tag is `ROW_FILTERED_AND_PROJECTED` the table is already in final output layout and is released directly; otherwise `gpu_ingestible::post_filter_and_project(...)` applies any pending row filter and projection. When the scan applies its dynamic filters itself (`enable_dynamic_filter_in_scan`, wired by the planner with a gate shared with the `DYNAMIC_FILTER` operator), `filter_and_project_with_dynamic_filters(...)` runs instead: the parquet ingestible evaluates the residual filter and the visible membership masks on the unmaterialized split and gathers only the survivors, so a pinned chunk is never deep-copied whole; the applied filters travel on the output (`scan_output_operator_data`) for the operator to pass through. Other formats fall back to `post_filter_and_project`.
